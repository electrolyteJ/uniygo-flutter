...
class OcgCoreNativeAdapter implements OcgCore {
  ...
  void initialize([ffi.DynamicLibrary? lib]) {
    if (_bindings != null) return;
    lib = lib ?? loadOcgCore();
    _bindings = OcgCoreBindings(lib);
  }
  ...
  void setScriptReader(ScriptReader? f) {
    ...
    _b.set_script_reader(ffi.Pointer.fromFunction<_scriptReaderNative>(
      _onScriptReader,
    ));
  }
  ...
  void setCardReader(CardReader? f) {
    ...
    _b.set_card_reader(ffi.Pointer.fromFunction<_cardReaderNative>(
      _onCardReader,
      0,
    ));
  }
  ...
  Future<void> preloadScriptAsync(String name) async {
    if (_scriptReader == null) {
      print('preloadScriptAsync called but scriptReader is null for name: $name');
      return;
    }
  ...
  void setMessageHandler(MessageHandler? f) {
    ...
    _b.set_message_handler(ffi.Pointer.fromFunction<_messageHandlerNative>(
      _onMessageHandler,
      0,
    ));
  }
  ...
  int queryCard(
    ...
    try {
      final ret =
          _b.query_card(pduel, playerid, location, sequence, queryFlag, ptr, useCache);
      if (ret > 0) {
        for (var i = 0; i < ret; i++) {
          out[i] = ptr[i];
        }
  ...
  int queryFieldCard(
    ...
    try {
      final ret =
          _b.query_field_card(pduel, playerid, location, queryFlag, ptr, useCache);
      if (ret > 0) {
        for (var i = 0; i < ret; i++) {
          out[i] = ptr[i];
        }
  ...
  void setResponseb(int pduel, Uint8List data) {
    final ptr = calloc<byte>(data.length);
    try {
...
typedef _scriptReaderNative = ffi.Pointer<byte> Function(
  ffi.Pointer<ffi.Char>,
  ffi.Pointer<ffi.Int>,
);

typedef _cardReaderNative = ffi.Uint32 Function(
  ffi.Uint32,
  ffi.Pointer<card_data>,
);

typedef _messageHandlerNative = ffi.Uint32 Function(
  ffi.IntPtr,
  ffi.Uint32,
);