...
class DuelSettings {
  ...
  static const defaults = DuelSettings();
...
/// 宿主 app 通过 [duelSettingsProvider.overrideWith] 注入带
/// SharedPreferences 持久化的实现（见 apps/duel_settings）。
class DuelSettingsNotifier extends Notifier<DuelSettings> {
  @override
  DuelSettings build() => DuelSettings.defaults;
...
final duelSettingsProvider =
    NotifierProvider<DuelSettingsNotifier, DuelSettings>(
      DuelSettingsNotifier.new,
    );

/// 打开全局设置弹窗的函数指针。默认 null（对局包不感知设置 UI 的具体实现），
/// 由宿主 app 注入 apps/duel_settings 的弹窗。
typedef ShowDuelSettingsDialog = void Function(BuildContext context);

final showDuelSettingsDialogProvider = Provider<ShowDuelSettingsDialog?>(
  (ref) => null,
);
