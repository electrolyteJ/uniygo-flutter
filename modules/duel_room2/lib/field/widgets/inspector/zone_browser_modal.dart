...
class ZoneBrowserModal extends StatelessWidget {
  ...
  /// 宽屏阈值：可用宽度 >= 1000 时沿用「左侧留 inspector + 居中 720 面板」布局。
  static const double _wideBreakpoint = 1000.0;

  @override
  Widget build(BuildContext context) {
    // 宽屏：左侧留出 inspector 区域（18 边距 + 324 面板 + 18 间隔），
    // 保证浏览区域时点击卡片能实时看到 inspector 更新。
    // 窄屏（< 1000）：遮罩与面板占满全宽（left: 0），
    // inspector 按页面 Stack 层序叠在本层之上。
    return Positioned.fill(
      child: LayoutBuilder(
        builder: (context, constraints) {
          final isWide = constraints.maxWidth >= _wideBreakpoint;
          final leftInset = isWide ? 360.0 : 0.0;
          return Padding(
...
@Preview(name: 'ZoneBrowserModal', size: Size(720, 560), brightness: Brightness.dark)
Widget previewZoneBrowserModal() => ZoneBrowserModal(
      zoneBrowserKey: 'self_grave',
      cards: const [
        ZoneBrowserCardEntry(sequence: 0, code: 89631139),
        ZoneBrowserCardEntry(sequence: 1, code: 46986414),
      ],
      selectedCardSequence: 0,
      onCardTap: (_, _) {},
      onClose: () {},
