...
/// 升起螺旋 → 展开盘旋 → 落地闪光 → 回调移除。
///
/// 配色（金属/关节/自发光）可参数化，供 cardlive 怪兽鉴赏页给不同
/// 怪兽换色复用；默认值即电子龙原配色。[loop] 为 true 时演出循环
/// 播放、不触发 [onCompleted]（鉴赏模式）。
class CyberDragonRig extends Object3D {
  CyberDragonRig({
    ...
    _metalMaterial = SpatialMaterial(
      albedoColor: metalColor ?? const Color(0xFFDDE4EE),
      metallic: 0.95,
      roughness: 0.25,
    );
    _jointMaterial = SpatialMaterial(
      albedoColor: jointColor ?? const Color(0xFF3A4250),
      metallic: 0.8,
      roughness: 0.5,
    );
    _glowMaterial = UnlitMaterial(albedoColor: glowColor ?? const Color(0xFF35E8FF));
  }
  ...
  /// 鉴赏模式：演出结束后从头循环，不移除自身、不回调 [onCompleted]。
  final bool loop;
  ...
  /// 金属材质（躯干）。
  late final SpatialMaterial _metalMaterial;

  /// 深色关节材质。
  late final SpatialMaterial _jointMaterial;

  /// 自发光材质（核心/眼部——「电子」感来源）。
  late final UnlitMaterial _glowMaterial;
