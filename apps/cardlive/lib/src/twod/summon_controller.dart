...
class SummonController extends FlameGame {
  ...
  Future<void> onLoad() async {
    ...
    // 光柱组件（全屏）
    _beam = BeamComponent(style: _theme)
      ..position = center
      ..size = size;
    add(_beam);

    // 粒子系统
    _particles = ParticleSystem(style: _theme)..position = center;
    add(_particles);

    // 光环组件
    _ring = RingComponent(style: _theme)..position = center;
    add(_ring);
      ...
      final textPainter = TextPainter(
        text: TextSpan(
          ...
          style: TextStyle(
            ...
            shadows: [
              Shadow(
                color: _theme.primaryColor.withValues(alpha: nameAlpha * 0.8),
                blurRadius: 12,
              ),
            ],
          ),
