...
class CardsLayerComponent extends Component {
  ...
  CardsLayerComponent({required this.camera, required this.zones, required this.state});
  ...
  void render(Canvas canvas) {
    ...
    for (int i = 0; i < 5; i++) {
      final pm = state.own.mon[i];
      if (pm != null) {
        cards.add(_CardRenderable(card: pm, zone: _zone(ZoneKind.monster, false, index: i), standing: pm.isAttack, id: i));
      }
      final ps = state.own.st[i];
      if (ps != null) {
        cards.add(_CardRenderable(card: ps, zone: _zone(ZoneKind.spell, false, index: i), standing: false, id: 10 + i));
      }
      final om = state.foe.mon[i];
      if (om != null) {
        cards.add(_CardRenderable(card: om, zone: _zone(ZoneKind.monster, true, index: i), standing: om.isAttack, id: 20 + i));
      }
      final os = state.foe.st[i];
      if (os != null) {
        cards.add(_CardRenderable(card: os, zone: _zone(ZoneKind.spell, true, index: i), standing: false, id: 30 + i));
      }
    }

    for (final isOpponent in [false, true]) {
      final field = isOpponent ? state.foe : state.own;
      final piles = <(ZoneKind, int, int)>[
        (ZoneKind.deck, field.deck.length, isOpponent ? 51 : 50),
        (ZoneKind.grave, field.grave.length, isOpponent ? 55 : 54),
      ];
  ...
  List<Offset>? _projectCardQuad(_CardRenderable r) {
    ...
    final yOffset = r.pileCount != null ? math.min(r.pileCount!, 20) * FieldLayout.stackStep : 0.0;
  ...
  void _drawCard(Canvas canvas, _CardRenderable r) {
    ...
    final isOwnMon = r.zone.isMonster && !r.zone.isOpponent;
    final selected = isOwnMon && state.selectedZone == r.zone.index;
    final targetable = r.zone.isMonster && r.zone.isOpponent && state.selectedZone != null;
    final faceUp = r.card.isFaceUp && r.pileCount == null;

    if (r.standing) _drawShadow(canvas, r);
    ...
    if (faceUp) {
      canvas.drawPath(
        ...
        Paint()
          ..shader = ui.Gradient.linear(quad[0], quad[2], [
            r.card.artColor.withValues(alpha: 0.5),
            r.card.artColor2.withValues(alpha: 0.9),
      ...
      canvas.drawPath(
        ...
        Paint()
          ..shader = ui.Gradient.linear(quad[0], quad[2], [
            const Color(0xFF3A2154),
            const Color(0xFF180C28),
    ...
    if (selected) {
      ...
      borderColor = const Color(0xFFE83A5A);
      borderWidth = 2;
    } else if (faceUp) {
      borderColor = r.card.typeColor.withValues(alpha: 0.7);
    } else {
      borderColor = const Color(0xFFD4A843).withValues(alpha: 0.5);
    }
    canvas.drawPath(
      path,
      Paint()
    ...
    if (selected) {
      canvas.drawPath(
        ...
        Paint()
          ..style = PaintingStyle.stroke
          ..strokeWidth = 5
          ..color = const Color(0xFFFFE4A0).withValues(alpha: 0.3 + 0.15 * math.sin(_time * 4))
    ...
    if (targetable) {
      canvas.drawPath(
        ...
        Paint()
          ..style = PaintingStyle.stroke
          ..strokeWidth = 4
          ..color = const Color(0xFFE83A5A).withValues(alpha: 0.25 + 0.15 * math.sin(_time * 5))
          ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 5),
      );
    }

    if (r.pileCount != null) {
      _drawPileCount(canvas, r, quad);
      return;
    }
  ...
  void _drawShadow(Canvas canvas, _CardRenderable r) {
    final c = r.zone.center;
  ...
  void _drawPileCount(Canvas canvas, _CardRenderable r, List<Offset> quad) {
    ...
    final cardH = ((quad[2].dy + quad[3].dy) / 2 - (quad[0].dy + quad[1].dy) / 2).abs();
    ...
    final tp = TextPainter(
      text: TextSpan(
        ...
        style: TextStyle(
          ...
          shadows: [Shadow(color: Colors.black.withValues(alpha: 0.9), blurRadius: 3)],
        ),
      ),
  ...
  void _drawCardText(Canvas canvas, _CardRenderable r, List<Offset> quad) {
    ...
    if (r.card.isMonster && cardH > 20) {
      final glyphSize = (cardH * 0.22).clamp(8.0, 28.0);
      final glyphTp = TextPainter(
        text: TextSpan(
          text: r.card.glyph,
          style: TextStyle(
            color: r.card.artColor.withValues(alpha: 0.9),
            fontSize: glyphSize,
            shadows: [Shadow(color: r.card.artColor.withValues(alpha: 0.6), blurRadius: 8)],
          ),
        ),
        textDirection: TextDirection.ltr,
      )..layout();
      glyphTp.paint(canvas, Offset(cx - glyphTp.width / 2, topY + cardH * 0.25));
    }

    if (r.card.isMonster && cardH > 25) {
      final statFontSize = (cardH * 0.07).clamp(4.0, 11.0);
      final statY = botY - statFontSize * 1.5;
      final atkTp = TextPainter(
        text: TextSpan(
          text: 'ATK/${r.card.atk}',
          style: TextStyle(color: const Color(0xFFFF8A6A), fontSize: statFontSize, fontWeight: FontWeight.bold),
        ),
        textDirection: TextDirection.ltr,
      )..layout();
      final defTp = TextPainter(
        text: TextSpan(
          text: 'DEF/${r.card.def}',
          style: TextStyle(color: const Color(0xFF6AAAFF), fontSize: statFontSize, fontWeight: FontWeight.bold),
        ),
        textDirection: TextDirection.ltr,
      )..layout();
    ...
    if (r.card.isMonster && cardH > 30) {
      final starSize = (cardH * 0.05).clamp(3.0, 8.0);
      final starTp = TextPainter(
        text: TextSpan(
          text: '★' * math.min(r.card.level, 10),
          style: TextStyle(color: const Color(0xFFFFD700), fontSize: starSize),
        ),
        textDirection: TextDirection.ltr,
      )..layout();
      starTp.paint(canvas, Offset(cx - starTp.width / 2, topY + cardH * 0.14));
    }
  }
  ...
  void _drawCardBack(Canvas canvas, List<Offset> quad) {
    ...
    final tp = TextPainter(
      text: const TextSpan(text: '☥', style: TextStyle(color: Color(0x99D4A843), fontSize: 12)),
      textDirection: TextDirection.ltr,
    )..layout();
    tp.paint(canvas, Offset(cx - tp.width / 2, cy - tp.height / 2));
  }
