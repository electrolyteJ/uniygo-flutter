  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        darkTextField(controller: _nameCtrl, label: '房间名称', icon: Icons.edit),
        const SizedBox(height: 10),
        PasswordField(controller: _pwCtrl, label: '房间密码', icon: Icons.lock),
        const SizedBox(height: 14),
        numberRow('初始 LP', _startLp, (v) => setState(() => _startLp = v)),
        numberRow('初始手牌', _startHand, (v) => setState(() => _startHand = v)),
        numberRow('每回合抽卡', _drawCount, (v) => setState(() => _drawCount = v)),
        ...
        const SizedBox(height: 6),
        Row(children: [
          checkRow('不检查卡组', _noCheckDeck, (v) => setState(() => _noCheckDeck = v ?? false)),
          const SizedBox(width: 16),
          checkRow('不切洗卡组', _noShuffleDeck, (v) => setState(() => _noShuffleDeck = v ?? false)),
        ]),
        ...
        const SizedBox(height: 10),
        connectButton(label: '创建房间', connecting: _connecting, onPressed: () => _create(context)),
      ]),
    );
  }
}
